...
<%
	...
	String whatContents = (String) session.getAttribute("whatContents");
	
	//경로 지정만을 위한 것으로, 변수 선언이 완료 되었으면 세션 해제
	session.removeAttribute("whatContents");
	//최초 작성-----------------------------------------------------------------------
	...
	filePath = config.getServletContext().getRealPath("/webtoonDB/list/" + whatContents);
	newPath = new File(filePath);
	if(!newPath.exists()){
		newPath.mkdir();
	}
	...
	//1차 DB 작성 (공간 확보)
	String listType = "normal"; //게시글일 경우 고정값
	String writerId = (String) session.getAttribute("memid"); //작성자
	String writerName = mul.getParameter("nickname"); //작성자
	...
	bean.setWhatcontents(whatContents);
	...
	String url = request.getContextPath() + "/boardList.jsp?boardCategoryName=" + whatContents;
	if(msg.equals("")){
		msg ="정상적으로 등록 되었습니다.";
	}
%>
