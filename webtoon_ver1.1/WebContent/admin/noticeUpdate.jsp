...
<div class="well well-sm">
	<b>
		...
		<a href="<%=contextPath%>/admin/noticeList.jsp">공지사항</a> &gt;
		공지 수정
	</b>
</div>

<div class="content-grid">
	<form method="post" action="noticeUpdatePro.jsp" enctype="multipart/form-data">
	<input type="hidden" name ="noticeNum" value="<%=listbean.getNum()%>">
	<h3 class="text-primary">공지사항 수정</h3>
	<table class="table">
		<tr>
			<th>분류</th>
			<td>
				<input class="form-control" type="text" <%if( listbean.getListType().equals("main")){%> value="메인 공지" <%}else{%> value="게시판 공지"<%}%> disabled>
			</td>
		</tr>
		<tr>
			<th>
			...
			<td><input type="text" class="form-control" name ="subject" value="<%=listbean.getSubject() %>" placeholder="[공지 제목]"></td>
		</tr>
		<tr>
			...
			<td>
				...
				<tr data-row-no=<%=i %> img-val=<%=imgArr[i]%>>
					<td>
						...
						<a href="#"
						onclick="window.open('<%=contextPath%>/webtoonDB/list/notice/<%=listbean.getNum() %>/<%=imgArr[i] %>','<%=imgArr[i] %>','width=500,height=600'); return false"
						><%=imgArr[i] %>
						</a>
					</td>
				</tr>
